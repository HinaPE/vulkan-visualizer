...
class TriangleRenderer:public IRenderer{public:
  ...
  void initialize(const EngineContext& e,const RendererCaps& c,const FrameContext&) override{
    dev=e.device;fmt=c.color_attachments.front().format;std::string dir(SHADER_OUTPUT_DIR);
    ...
    const VkDynamicState dyns[2]={VK_DYNAMIC_STATE_VIEWPORT,VK_DYNAMIC_STATE_SCISSOR};VkPipelineDynamicStateCreateInfo ds{VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO};ds.dynamicStateCount=2;ds.pDynamicStates=dyns;
  ...
  void on_imgui(const EngineContext&,const FrameContext& f) override{
    ImGui::Begin("Controls");ImGui::Text("ex01_imgui_minimal_ui");ImGui::SliderFloat3("clear",clear_,0.0f,1.0f);ImGui::Text("Extent %u x %u",f.extent.width,f.extent.height);ImGui::Text("FPS %.1f",ImGui::GetIO().Framerate);ImGui::Text("Press F12 to screenshot");ImGui::End();
    ImGui::Begin("Log");ImGui::TextUnformatted("Dock panels freely. This example checks input/DPI and UI plumbing.");ImGui::End();
  }
private: VkDevice dev{};VkPipelineLayout layout{};VkPipeline pipe{};VkFormat fmt{};float clear_[3]{0.05f,0.07f,0.12f};};

int main(){try{VulkanEngine e;e.configure_window(1280,720,"ex01_imgui_minimal_ui");e.set_renderer(std::make_unique<TriangleRenderer>());e.init();e.run();e.cleanup();}catch(const std::exception& ex){fprintf(stderr,"Fatal: %s\n",ex.what());return 1;}return 0;}
